...
def get_count():
    ...
    try:
        ...
        # જો નવો દિવસ હોય
        if last_date != today:
            # જો ગઈકાલે જાપ કર્યા હતા, તો જ સ્ટ્રીક વધારો, નહીતર ૦ કરી દો
...
def update_count():
    ...
    try:
        # પેલા ચેક કરો કે ડેટાબેઝમાં હાલની સ્ટ્રીક કેટલી છે
        res = supabase.table("jap_counter").select("streak").eq("user_id", user_id).execute()
        streak = res.data[0]['streak'] if res.data else 0

        # જો આજે પહેલી વાર જાપ થયા અને સ્ટ્રીક 0 છે, તો તેને 1 કરો
        if curr > 0 and streak == 0:
            streak = 1
            supabase.table("jap_counter").update({
                "current_count": curr, "total_count": total, "last_date": today, "streak": streak
            }).eq("user_id", user_id).execute()
        else:
            supabase.table("jap_counter").update({
